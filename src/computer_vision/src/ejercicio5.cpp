...
const int MAX_LINE_WIDTH = 3;
const int MAX_IMAGE_ROTATION = 90;
...
const char * trackbar_4_text = "Width";
const char * trackbar_5_text = "Rotation";
...
int TRACKBAR_ITERATIONS, TRACKBAR_DISTANCE, LINE_WIDTH;
...
cv::Mat print_3d_to_2d_proyection_lines(cv::Mat input)
{
  for (int i = 0; i <= TRACKBAR_DISTANCE; i++) {
    cv::Point p1 = proyect_3d_to_2d(LINE_WIDTH, 0.0, double(i));
    cv::Point p2 = proyect_3d_to_2d(-LINE_WIDTH, 0.0, double(i));
...
void initialize_user_interface()
{
  ...
  cv::createTrackbar(trackbar_4_text, p5_window_name, nullptr, MAX_LINE_WIDTH, 0);
  cv::createTrackbar(trackbar_5_text, p5_window_name, nullptr, MAX_IMAGE_ROTATION, 0);
  ...
  cv::setTrackbarPos(trackbar_4_text, p5_window_name, 0);
  cv::setTrackbarPos(trackbar_5_text, p5_window_name, 45);
...
cv::Mat rotate_image(cv::Mat src, double angle, cv::Point center){
  cv::Mat rotation_dst;
  double scale = 1;
  cv::Mat rot_mat = cv::getRotationMatrix2D(center, -angle, scale);
  cv::warpAffine(src, rotation_dst, rot_mat, src.size());
  return rotation_dst;
}
...
cv::Mat image_processing(const cv::Mat in_image)
{
  ...
  if (!initialized_img) {
    ...
    rotation_center = cv::Point(in_image.cols / 2, in_image.rows / 2);
  }
  ...
  LINE_WIDTH = cv::getTrackbarPos(trackbar_4_text, p5_window_name);
  IMAGE_ROTATION = (double)(cv::getTrackbarPos(trackbar_5_text, p5_window_name) - 45);
  ...
  switch (CAMERA_MODE) {
    ...
    case CLEAN:
    default:
      out_image = rotate_image(in_image,IMAGE_ROTATION, rotation_center);
      cv::circle(out_image, rotation_center, 3, cv::Scalar(0, 255, 0), 3, cv::LINE_AA);
      Points.clear();
      break;
  }
