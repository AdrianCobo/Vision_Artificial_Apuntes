...
{
public:
  ComputerVisionSubscriber() : Node("opencv_subscriber")
  {
...
cv::Mat bgr_2_cmy(cv::Mat src)
{
  ...
  for (int i = 0; i < src.rows; i++) {
    for (int j = 0; j < src.cols; j++) {
      ...
      three_channels[0].at<uchar>(i, j) = 255 - (uint)three_channels[0].at<uchar>(i, j);
      three_channels[1].at<uchar>(i, j) = 255 - (uint)three_channels[1].at<uchar>(i, j);
      three_channels[2].at<uchar>(i, j) = 255 - (uint)three_channels[2].at<uchar>(i, j);
    }
  }
...
cv::Mat bgr_2_hsi(cv::Mat src)
{
  ...
  for (int i = 0; i < src.rows; i++) {
    for (int j = 0; j < src.cols; j++) {
      B = (double)three_channels[0].at<uchar>(i, j);
      G = (double)three_channels[1].at<uchar>(i, j);
      R = (double)three_channels[2].at<uchar>(i, j);
      ...
      S = 1.0 - (3.0 / (R + G + B)) * ((double)RGBmin.at<uchar>(i, j) / 255.0);
...
cv::Mat image_processing(const cv::Mat in_image)
{
  ...
  // change the image format if necessary
