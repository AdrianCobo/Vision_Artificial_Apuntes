...
pcl::PointCloud<pcl::PointXYZRGB> green_lines_filter(pcl::PointCloud<pcl::PointXYZRGB> in_pointcloud)
{
...
pcl::PointCloud<pcl::PointXYZRGB> paint_line(
  pcl::PointCloud<pcl::PointXYZRGB> dst_pointcloud, int r, int g, int b)
{
  pcl::PointCloud<pcl::PointXYZRGB> line;

  for (size_t j = 0; j < dst_pointcloud.size(); j++) {
    pcl::PointXYZRGB p = dst_pointcloud.points[j];
    p.r = r;
    p.g = g;
    p.b = b;
    line.push_back(p);
...
pcl::PointCloud<pcl::PointXYZRGB> pcl_paralell_lines(pcl::PointCloud<pcl::PointXYZRGB> in_pointcloud)
{
  ...
  pcl::PointCloud<pcl::PointXYZRGB> auxiliar;
  pcl::SACSegmentation<pcl::PointXYZRGB> seg;
  pcl::ExtractIndices<pcl::PointXYZRGB> extract; // Create the filtering object
  float x_axe, z_axe;

  pcl::PointCloud<pcl::PointXYZRGB>::Ptr lines_point_cloud(in_pointcloud.makeShared());
  int nr_points = (int)lines_point_cloud->size();
  seg = initialice_segmentator();

  // While 30% of the original cloud is still there
  while (lines_point_cloud->size() > 0.01 * nr_points) {
    // Segment the largest planar component from the remaining cloud
    seg.setInputCloud(lines_point_cloud);
    ...
    x_axe = coefficients->values[3];
    z_axe = coefficients->values[5];

    // Extract the inliers
    extract.setInputCloud(lines_point_cloud);
    extract.setIndices(inliers);
    extract.setNegative(false);
    extract.filter(*cloud_p);

    if (x_axe > z_axe){
      auxiliar = paint_line(*cloud_p, 0, 0, 255);
      result = result + auxiliar;
    }else{
      auxiliar = paint_line(*cloud_p, 255, 0, 0);
      result = result + auxiliar;
    }
    ...
    lines_point_cloud.swap(cloud_f);
  }
...
pcl::PointCloud<pcl::PointXYZRGB> pcl_processing(
  const pcl::PointCloud<pcl::PointXYZRGB> in_pointcloud)
{
  pcl::PointCloud<pcl::PointXYZRGB> green_lines, lines_painted, out_pointcloud;
     
  green_lines = green_lines_filter(in_pointcloud);
  lines_painted = pcl_paralell_lines(green_lines);
  out_pointcloud = draw_floor_cubes(lines_painted);
